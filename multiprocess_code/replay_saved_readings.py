...
def text_reader_process(dest_queue):
    """
    ✅ SEPARATE PROCESS - Runs on its own CPU core
    Reads angles from text file + KP/KD from JSON, sends complete config via queue
    """
    print(f"[Text Process PID {mp.current_process().pid}] Starting...")
    ...
    # check if length of all arrays are same
    while not (len(m1) == len(m2) == len(m3)):
        print("[Text Process] Error: Data series lengths do not match.")
        time.sleep(1.5)

    len_data = len(m1)
    print(f"[Text Process] Loaded {len_data} data points from {filename}")

    counter = 0

    time.sleep(5)  # Initial delay before starting
    ...
    try:       
        while counter < len_data:
            try:
                ...
                angle_array = np.array([m1[counter], 0.0, m3[counter]]) 
                ...
                if changed:
                    print(f"\n\n\n[Text CHANGED] angles={angle_array.tolist()}° KP={kp_array.tolist()} KD={kd_array.tolist()}")
                    prev_angles = angle_array.copy()
                    prev_kp = kp_array.copy()
                    prev_kd = kd_array.copy()      

                
                time.sleep(0.2)  # Adjust read frequency as needed
                
            except Exception as e:
                print(f"[Text Process] Read error: {e}")
                time.sleep(0.1)  # Brief pause on error
                
    except KeyboardInterrupt:
        print("[Text Process] Interrupted")
    finally: 
        print("[Text Process] Shutdown")
            ...
            for t in loop: 
                ...
                if move_active and movestarttime:
                    ...
                    if time.time()>(last_print+0.2): # print every 0.1 second
                        last_print = time.time()
                        print(f"t {t:.1f} M {motor_ids}:  Temp={current_temp_deg.tolist()}°C | Target={np.round(targets_deg, 2)} | Actual={np.round(current_positions_deg, 2)} | TrackEr={np.round(np.abs(current_positions_deg - targets_deg), 2)} | FinalEr={np.round(np.abs(current_positions_deg - dest_deg), 2)} | KP={KP} KD={KD} Vel={np.round(np.degrees(targets_vel_rad), 1)}°/s")
