...
x = 9.094
z_start, z_end = 9, -4 # right to left co-ordinates of bots leg for movement
y_start, y_end = 37, 27 # max up and max low of bots leg
...
def run_trajectory(dev1, dev2, dev3):
    ...
    dev1.set_impedance_gains_real_unit(K=6.0, B=3.0)
    dev2.set_impedance_gains_real_unit(K=6.0, B=3.0)
    dev3.set_impedance_gains_real_unit(K=6.0, B=3.0)  # Adjust as needed
    
    print("Starting trajectory demo for three motors. Press ctrl+C to quit.")

    trajectory_update_interval = 0.01  # seconds, e.g., update trajectory every 0.5s
    ...
    try:
        for t in loop:
            ...
            if t < 1.0:
            	print("Holding at initial position")
            	dev1.position = 0.0
            	dev2.position = 0.0
            	dev3.position = 0.0
            	got_to_home = True
            	...
            	if t>time_at_home+1: 
            		print("Holding at home position")
            		coord_idx = 0
            		x, y, z = coords[coord_idx] # Move to first trajectory point directly
            		coord_idx = 1
            		theta1, theta2, theta3 = ik.inverse_kinematics(x, y, z, L1, linkConst, L2, L3)
            		print("At Home: ", -theta1, -theta2, -theta3)
            		dev1.position = 0
                ...
                if update_counter % iterations_per_update == 0:
                    ...
                    theta1, theta2, theta3 = ik.inverse_kinematics(x, y, z, L1, linkConst, L2, L3)
                    print("At Runs: ", -theta1, -theta2, -theta3)
                    coord_idx += 1
                    if coord_idx >= len(coords):
                        coord_idx = 0
                        got_to_home = True
                        time_at_home = t
                        print("Go at home position")
                    else:
                        got_to_home = False
